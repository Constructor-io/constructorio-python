...
class TestConstructorIO(TestCase):
    ...
    def test_put_automatically_generated_refined_queries(self):
        with my_vcr.use_cassette(
                "fixtures/ac.cnstrc.com/put-refined-queries-autogenerated-success.yaml",
                match_on=vcr_matchers):
            constructor = ConstructorIO(**HTTPS_ARGS)
            resp = constructor.put_automatically_generated_refined_queries(
                refined_queries=[
                    {
                        "query": "<query 1>",
                        "blacklist_rules": [],
